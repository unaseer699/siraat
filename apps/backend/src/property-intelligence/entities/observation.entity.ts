...
// originating context, not Market Intelligence"). Trust and Construction
// Intelligence each own an identical-shaped table in their own schema; this
// one is Property Intelligence's copy — no shared table, no cross-schema FK.
@Index(['entity_ref', 'metric'])
@Entity({ name: 'observations', schema: 'property_intelligence' })
export class ObservationEntity {
  @PrimaryGeneratedColumn('uuid')
  id: string;

  // UUID string reference to the Society (or other in-context entity) this
  // observation is about — no SQL FK (Law 1: zero cross-context foreign keys).
  @Column('uuid')
  entity_ref: string;

  // e.g. "confidence_score"
