...
export default function AddClaimPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      const res = await addClaimToSociety(societyId, {
        claim: claim.trim(),
        ...
        {result ? (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '12px', fontWeight: 600 }}>
              <Link href={`/admin/society/${societyId}/add-claim`} onClick={() => setResult(null)} style={{ fontSize: '13px' }}>
                Add another claim →
              </Link>
              <Link href={`/society/${societyId}`} style={{ fontSize: '13px' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={fieldGroupStyle}>
              ...
              <input
                ...
                placeholder='e.g. "Blue Bell Block — Illegal Scheme Notice"'
                required
                style={inputStyle}
              />
            </div>
