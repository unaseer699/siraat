...
export default function AddClaimPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setError(null);
    try {
      const res = await addClaimToSupplier(supplierId, {
        claim: claim.trim(),
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '48px 16px 32px' }}>
      <article style={{ maxWidth: '560px', width: '100%', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <Link href="/admin/new-supplier" style={{ fontSize: '14px', color: 'var(--muted)' }}>
          ← Back to suppliers
        </Link>
        ...
        {result ? (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '12px', fontWeight: 600 }}>
              <Link href={`/admin/supplier/${supplierId}/add-claim`} onClick={() => setResult(null)} style={{ fontSize: '13px' }}>
                Add another claim →
              </Link>
            </div>
          </div>
